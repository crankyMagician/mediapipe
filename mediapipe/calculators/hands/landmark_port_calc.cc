...
// packets will be ignored (allowing LandMarkPortCalc to be used to
// test internal behavior).  Any options may be specified and will be
// ignored.
class LandMarkPortCalc : public CalculatorBase {
 public:
  static absl::Status GetContract(CalculatorContract* cc) {
    if (!cc->Inputs().TagMap()->SameAs(*cc->Outputs().TagMap())) {
      return absl::InvalidArgumentError(
          "Input and output streams to LandMarkPortCalc must use "
    ...
    if (cc->OutputSidePackets().NumEntries() != 0) {
      if (!cc->InputSidePackets().TagMap()->SameAs(
        ...
        return absl::InvalidArgumentError(
            "Input and output side packets to LandMarkPortCalc must use "
            "matching tags and indexes.");
      }
      for (CollectionItemId id = cc->InputSidePackets().BeginId();
           id < cc->InputSidePackets().EndId(); ++id) {
...
REGISTER_CALCULATOR(LandMarkPortCalc);
